...
def cancel_all_bids_agent_except(market, current_agent, current_order):
    ...
    cancel_orders([o for o in order.active_buy_orders[market.id] if (o.agent.name == current_agent.name and o.id != current_order.id)])
        
def cancel_all_offers_agent_except(market, current_agent, current_order):
    """Remove all bids agent except"""
    cancel_orders([o for o in order.active_sell_orders[market.id] if (o.agent.name == current_agent.name and o.id != current_order.id)])