...
class market():
    ...
    def get_last_order(self):
        ...
        if len(ordr.order.history_order[self.id]) > 0:
            last_order_id = max(ordr.order.history_order[self.id].keys())
            last_order = ordr.order.history_order[self.id][last_order_id]
        # Else --> return 0
        else:
            last_order = 0     
        return last_order
    ...
    def print_last_cancellations(self, last_order_cancelled_id):
        for c in cx.cancellation.history[self.id]:
    ...
    def print_last_orders(self, last_order_id):
        for counter, o in ordr.order.history_order[self.id].items():
    ...
    def print_last_transactions(self, last_transaction_id, max_number_transactions = 5):
        for t in tr.transaction.history[self.id][-max_number_transactions:]:
    ...
    def show_orderbook(self, depth = 10):
        ...
        for sell_order in sorted(ordr.order.active_sell_orders[self.id], key = lambda x: (x.price, x.id), reverse=True)[(-1 * depth):]:
            print(width_orderbook * "." + " " + str(sell_order))
        for buy_order in sorted(ordr.order.active_buy_orders[self.id], key = lambda x: (-1 * x.price, x.id))[:depth]:
    ...
    def show_orderbook_orders_aggregated(self, depth = 10):    
        ...
        for o in sorted(ordr.order.active_sell_orders[self.id], key = lambda x: (x.price, x.id)):
        ...
        for o in sorted(ordr.order.active_buy_orders[self.id], key = lambda x: (-1 * x.price, x.id)):
    ...
    def show_orders_agent(self, current_agent, depth_orderbook = 20):        
        min_level_sell_side = min([o.price for o in ordr.order.active_sell_orders[self.id]]) if (len(ordr.order.active_sell_orders[self.id]) > 0) else self.maxprice
        max_level_buy_side = (max([o.price for o in ordr.order.active_buy_orders[self.id]]) + self.ticksize) if (len(ordr.order.active_buy_orders[self.id]) > 0) else self.minprice
        ...
        for o in ordr.order.active_sell_orders[self.id]:
        ...
        for o in ordr.order.active_buy_orders[self.id]:
            if o.agent.name == current_agent.name:
    ...
    def plot_price(self, skip_first_transactions = 0, skip_last_transactions_after = 99999):
        df = pd.DataFrame(tr.transaction.history_list[self.id], columns = ["id", 
                                                                           "time", 
                                                                           "price"])
        df = df[["id", "price"]]
    ...
    def plot_positions(self, agents = []):
        ...
        for a in self.agents:
            if a.name in agents:
                df = pd.DataFrame(tr.transaction.history_market_agent[self.id, a.name], columns = ["id", 
                                                                                                   "running_position", 
                                                                                                   "running_profit"])
                df = df.set_index("id")
